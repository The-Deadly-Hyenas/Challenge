...
def my_score_function(y_true, y_pred):
    ...
    for sklearn use this:
        from sklearn.metrics import make_scorer
        score = make_scorer(my_score_function, greater_is_better=True)
        score(classifier, X, y) # same as my_score_function(y, classifier.predict(X))
    ...
    x = -1e10  # padding to make indexing possible
    gain_matrix = np.array([[5, -5, -5, 2],
                            [-5, 10, 2, -5],
                            [-5, 2, 10, -5],
                            [2, -5, -2, 5]])  # actual and predicted = "happy": revenue_matrix[1,1] => 5

    score = gain_matrix[y_true, y_pred].sum()
